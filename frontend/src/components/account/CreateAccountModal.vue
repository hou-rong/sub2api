<template>
  ...
  >
    ...
    >
      ...
      <div v-if="form.platform === 'kimi'">
        ...
        <div class="mt-2 grid grid-cols-1 gap-3 sm:grid-cols-2" data-tour="account-form-type">
          ...
          >
            ...
            <div>
              <span class="block text-sm font-medium text-gray-900 dark:text-white">API Key</span>
              <span class="text-xs text-gray-500 dark:text-gray-400">{{ t('admin.accounts.types.kimiApiKey') }}</span>
            </div>
          </button>
        </div>
        <p class="mt-2 text-xs text-gray-500 dark:text-gray-400">
          {{ t('admin.accounts.oauth.kimi.authMethodsHint') }}
        </p>
      </div>

      <!-- Account Mode Selection (Kimi / Zhipu / DeepSeek) -->
      <div v-if="isCNPlatform && (form.platform !== 'kimi' || accountCategory === 'apikey')">
      ...
      <div
        v-if="isCNPlatform && (form.platform !== 'kimi' || accountCategory === 'apikey')"
        class="mt-4"
      >
      ...
      <div v-if="form.type === 'apikey' && form.platform !== 'antigravity'" class="space-y-4">
        ...
        <div>
          ...
          <input
            ...
            required
            class="input font-mono"
            :placeholder="form.platform === 'grok' ? 'xai-...' : apiKeyValuePlaceholder"
          />
...
function selectKimiAccountCategory(category: 'oauth-based' | 'apikey') {
  ...
  // 兼容原 Kimi 分支的“API Key = Kimi Coding”默认语义；用户仍可在下方切换为 PayG。
  accountMode.value = 'coding'
  apiProtocol.value = 'chat_completions'
  apiKeyBaseUrl.value = defaultCNBaseUrl('kimi', accountMode.value, apiProtocol.value)
}
