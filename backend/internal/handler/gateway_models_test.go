...
// Kimi keeps the fork's Coding catalog; Zhipu and DeepSeek retain the
// Claude-compatible defaults used by upstream v0.1.179.
func TestDefaultModelIDsForPlatform_CNProvidersPreservePlatformDefaults(t *testing.T) {
	want := make([]string, 0, len(claude.DefaultModels))
	for _, model := range claude.DefaultModels {
		want = append(want, model.ID)
	}
	require.Equal(t, kimi.DefaultModelIDs(), defaultModelIDsForPlatform(service.PlatformKimi))
	for _, platform := range []string{service.PlatformZhipu, service.PlatformDeepseek, service.PlatformMiniMax} {
