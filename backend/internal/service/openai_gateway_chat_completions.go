...
func (s *OpenAIGatewayService) forwardAsChatCompletions(
	...
	// Keep the typed request in sync with the final body after account/group
	// Fast policy rewriting so downstream usage billing observes the outbound
	// service tier rather than the client-supplied value.
	...
	if handleErr == nil && result != nil {
		if tier := resolvedOpenAIUpstreamServiceTier(c, finalServiceTier); tier != nil {
