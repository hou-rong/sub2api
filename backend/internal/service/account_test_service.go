...
func (s *AccountTestService) TestAccountConnection(c *gin.Context, accountID int64, modelID string, prompt string, mode string, opts ...AccountTestOptions) error {
	...
	// Kimi Coding OAuth and legacy Coding API-key accounts retain the dedicated
	// kimi.com transport. Explicit PayG/API-protocol accounts use the generic
	// CN-provider paths introduced in v0.1.179.
	if account.Platform == PlatformKimi && account.UsesKimiCodingAPI() {
